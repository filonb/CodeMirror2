...
window.CodeMirror = (function() {
  ...
  function CodeMirror(place, givenOptions) {
    ...
    var instance = wrapper.CodeMirror = {
      ...
      blur: function(){input.blur(); onBlur(); fastPoll();},
...
/*
 * NCLab fix
 * Commented out - fixing a strange bug:
 * when CM is out of focus, then moved by an outer scrollbar in a way
 * that last selected cursour is out of view: then clicking in creates an unwanted selection
 */
...
/*
 * NCLab tweak
 * added a helper function to easily find outer element inside which CM sits
 * as we use it without CM scrollbars we have to take care of scrolling inside 
 * an ExtJS window
 */
...
/*
 * NCLab tweak
 * we need to properly scroll CM as it is inside a window with scrollbar
 * basicly we change calculation of bounding rectangle and fix strange behavior of 
 * scrollIntoView inside ExtJS desktop
 */    
      var outer = getNthAncestor(wrapper, 10); //mysterious 8 to get out of CM wrapping divs
      var outerBox = {top: 0, bottom: window.innerHeight};  // just setup defaults in case outer is not defined
      if (outer) outerBox = outer.getBoundingClientRect(); // get correct bounding rectangle
      if (coords.y + box.top < outerBox.top) doScroll = true; // do not compare to 0 but top of the outer
      else if (coords.y + box.top + textHeight() > (outerBox.bottom || window.innerHeight || document.documentElement.clientHeight)) doScroll = false;
      if (doScroll != null) {
        ...
        if (doScroll) {// calling scrollIntoview(true) causes ExtJS desktop failure
          /*  other solution to add an element to the bottom of outer element and scroll there
          var stop = elt("pre", null, null, "position: absolute; bottom: 0; background-color:red; width: 12px; height: " + lh + "px");
          outer.appendChild(stop);
          */
          var lh = textHeight();
          var cur_l = (sel.inverted ? sel.from : sel.to).line;
          var go_to_l = Math.min(Math.floor((outerBox.bottom - outerBox.top)/lh) + cur_l - 1, doc.size - 1);
          var l = lineDiv.childNodes[go_to_l];
          l = l || cursor;
          l.scrollIntoView(false);  // we scroll to last line which is above outerBox.bottom - not 100% correct
//          stop.scrollIntoView(false);
//          outer.removeChild(stop);
        } else {  // scrolling down works correctly
